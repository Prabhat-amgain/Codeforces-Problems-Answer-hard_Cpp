...
Kamilka has a flock of n sheep, the i-th of which has a beauty level of ai. All ai are distinct. Morning has come, which means they need to be fed.
Kamilka can choose a non-negative integer d and give each sheep d bunches of grass.
...
If the beauty levels of these two sheep are x and y (after they have been fed),
then Kamilka's pleasure from the walk is equal to gcd(x,y), where gcd(x,y) denotes the greatest common divisor (GCD) of integers x and y.
...
The second line of each test case contains n distinct integers a1,a2,…,an (1≤ai≤109) — the beauty levels of the sheep.
...
10
Note
In the first test case, d=1 works. 
In this case, the pleasure is gcd(1+1, 1+3)=gcd(2, 4)=2.
